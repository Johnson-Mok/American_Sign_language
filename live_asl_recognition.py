...
saved_model_path = "models/ASL_model_20230617_185330.h5"
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            x_min, y_min, x_max, y_max = get_hand_bounding_box(hand_landmarks, frame.shape[1], frame.shape[0], margin=10)

            # Extract hand region from the frame
            hand_roi_img = frame[y_min:y_max, x_min:x_max]
            hand_roi = cv2.resize(hand_roi_img, (40, 40))
            hand_roi = hand_roi.astype(np.float32) / 255.0
            hand_roi = np.expand_dims(hand_roi, axis=0)

            # Make a prediction on the hand region
